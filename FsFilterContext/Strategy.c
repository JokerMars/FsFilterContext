...
BOOLEAN IsSecretProcess(PFILE_TYPE_PROCESS current, PCHAR procName)
{
	if (current == NULL || procName == NULL)
	{
		return FALSE;
	}

	PPROCESS_INFO proc_info = current->processInfo;
	while (proc_info)
	{
		if (strncmp(proc_info->procName, procName, strlen(procName)) == 0)
		{
			return TRUE;
		}
